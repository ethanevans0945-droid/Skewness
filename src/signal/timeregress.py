...
df = df.sort(["barrid","date"]).with_columns(pl.col('turnover').shift(days).rolling_mean(window_size=days).over('barrid').alias(f'per1turn'))
df = df.sort(["barrid","date"]).with_columns(pl.col('turnover').rolling_mean(window_size=days).over('barrid').alias(f'per2turn'))
